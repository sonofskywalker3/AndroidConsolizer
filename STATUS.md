...
**Current version:** **3.7.64** (committed locally). **Last GitHub release: v3.7.0** (2026-05-14). **Everything from v3.7.1 → v3.7.64 is committed locally but NOT pushed** — a publish is owed whenever the user gives the explicit go-ahead (push + GitHub release + README + Nexus).

**Session 2026-05-30 — #18 museum donation + rearrange controller support (IN PROGRESS, v3.7.58→v3.7.64):**
- **Donation: DONE, device-confirmed working on G Cloud (v3.7.61).** Brainstorm+plan are in `docs/superpowers/specs/2026-05-29-museum-donation-design.md` and `docs/superpowers/plans/2026-05-29-museum-donation.md`, BUT the design's root-cause premise (snap chain gated on SnappyMenus=false on Android) was **wrong on G Cloud** — diagnostics proved SnappyMenus field+property were already true and the mechanics worked; the real blocker was **no visible cursor** (`drawMouse` suppressed because control type reads TOUCH). Fix = draw cursor tile 44 in a `MuseumMenu.draw` postfix. Full root-cause writeup in user memory `museum-donation-controller-rootcause`.
- **Rearrange: DONE, device-confirmed "works/better" (v3.7.63).** Needed `reOrganizing=true` (ctor postfix) so `receiveKeyPress` grid-navigates instead of walking the hidden inventory. `rearrangeMode`/`reOrganizing` are Android-only fields → reflected.
- **Right-stick free-pan (v3.7.64): DEPLOYED, awaiting user confirmation.** For the cramped far-right/top tiles at default zoom on G Cloud. `MuseumMenu.update` postfix → `Game1.panScreen` from the right stick (both axes).
- All in one file `Patches/MuseumMenuPatches.cs`; toggle `EnableMuseumDonationController` (default true) in `ModConfig`/GMCM; restore via `ModEntry.OnMenuChanged`.
- **REMAINING:** (1) confirm v3.7.64 pan on device; (2) **strip the transient `[MuseumMenu/diag]` instrumentation** (Diag_Ctor/ReceiveKeyPress/ReceiveLeftClick/Update postfixes, `DiagState`, `_lastLoggedState`, `_invCurrentlySelectedItemField`, the diag patch block, and the `[MuseumMenu/diag]` logs in `EngageSnappyForMuseum`/restore) → clean v3.7.65; (3) docs close-out (`DONE.md` entry, mark #18 done in `TODO.md`/`STATUS.md`). Keep the SnappyMenus field+property force (confirmed no-op on G Cloud but cross-device insurance).
