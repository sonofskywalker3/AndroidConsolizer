...
**Current version:** **3.8.0 — SHIPPED 2026-05-31** 🎉 (GitHub: pushed to `master` + release `v3.8.0` with ZIP; Nexus: file auto-published by the release workflow + description & version updated to 3.8.0). **Last GitHub release: v3.8.0.** Working tree is clean and fully pushed — nothing is owed.

### ▶ FRESH AGENT START HERE
- **v3.8.0 (Console Parity: Quick Wins) is DONE and shipped.** Don't reopen it.
- **Next milestone: v3.9.0 — Console Parity: Big Systems** → **#25 tool charging while moving** and **#25b slingshot aim** (the explicit right-stick-rule exception). See `TODO.md` for both items' notes. Brainstorm-first per the Superpowers workflow.
- **Start the next change at v3.8.1** (next 0.0.1) unless the user requests otherwise; bump `manifest.json` BEFORE building, one change per commit.
- Publishing is automated now: **creating a GitHub release auto-publishes the file to Nexus** via `.github/workflows/publish-nexus.yml`. The mod-page **description + version** can be updated remotely with `release-notes/nexus-update.mjs` (Playwright-node, dedicated Chrome profile). The per-version **changelog** is the only manual step (no API; modern Nexus has no changelog editor route — it's in Files → Version management). See memory `nexus-publishing`.
- Primary test device: **G Cloud** on wireless ADB `192.168.228.87:5555`. Test save **Cheatside** (CJB Item Spawner = key `I`). Deploy/logs via `../SyncdewValley/sync.ps1`.
