...
### 81. Bookseller (and other barter shops) only deliver 1 of a trade's output stack — 🔧 FIXED v4.0.1 (PENDING PLAYTEST)
- **✅ Root cause found + fixed v4.0.1.** AC's A-button purchase path set `newItem.Stack = quantity` (the buy-count), dropping the salable's own per-unit `Stack`. Vanilla delivers `stockToBuy * item.Stack` (decompile `ShopMenu.tryToPurchaseItem:1621`); the salable's `Stack` is the bundle size (bookseller trades 1 book for a Hardwood salable whose `Stack` is 20). Fix in `Patches/ShopMenuPatches.cs`: `newItem.Stack = quantity * Math.Max(1, selectedItem.Stack)`. Normal shops are `Stack=1` → no change. **Needs device playtest:** trade a book at the bookseller and confirm you receive the full stack (e.g. 20 Hardwood).
- **Report:** *NightMareBalon (Nexus, 13 Jun 2026)*: buying/trading quantity items behaves oddly — "bookseller: Woody's Secret x1 ⇒ TradeTo ⇒ Hardwood x20, but all I got was one piece of hardwood." Author acknowledged 14 Jun ("just back from vacation, will start tomorrow").
- **What this is:** the bookseller is a `ShopMenu` whose stock trades an item for an item (not gold). The trade should grant the full output stack (e.g. 20 Hardwood) but the controller purchase path delivers only 1. Almost certainly AC's A-button purchase wiring forcing `stack = 1` / not reading the stock entry's output quantity, rather than vanilla shop math.
- **Investigate first (diagnostic-first rule):** read decompiled Android `ShopMenu.cs` purchase path (`purchaseItem` / `tryToPurchaseItem` and how trade-currency `ItemStockInformation` carries the output stack) and compare to AC's `Patches/ShopMenuPatches.cs` A-purchase handler — does AC override the purchased stack count, or call purchase with `stock=1` regardless of the entry's stack? Reproduce on device (grant Woody's Secret, trade at bookseller) before fixing. **Tie-in:** memory `android-shopmenu-needs-real-item-salables` — Android `ShopMenu.draw` casts salables to `Item`; verify the fix uses real-item stock quantity, not a synthesized count.
