...
- **41b. NOT FIXED (v3.4.68 attempted)** — Bundle reward still redeemable multiple times. `releaseLeftClick` postfix nulls presentButton after click, but presentButton is RECREATED by `checkForRewards()` each time the menu re-opens. Root cause: `bundleRewards` may not be properly cleared by `rewardGrabbed`. v3.4.71 fixes the diagnostic to properly read `Game1.netWorldState.Value.BundleRewards` (was using Field instead of Property). Need logs from v3.4.71 to confirm.
