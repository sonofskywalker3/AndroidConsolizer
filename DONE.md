...
- **✅ SHIPPED in v4.0.0 (2026-06-05, GitHub + Nexus) — mod now FEATURE COMPLETE.** #12/#62/#79/#76/#77 all done (below); pre-release audit gated the last stray logs and made #77 persistence robust (v3.9.19→v3.9.23). `EnforceTick` intentionally retained (redundant-but-harmless). See `docs/superpowers/specs/2026-06-05-handoff-right-stick-cursor-wip.md` + memory `androidconsolizer-feature-complete-v4`.

### #76 Tool-Hit Box Reverts to Facing Tile on Cursor Fade — DONE (v3.9.17, device-verified G Cloud 2026-06-05)
- **Symptom:** with the right-stick cursor, the "always show tool hit location" box (`Farmer.draw` tile 29) followed the cursor, but after the cursor faded and you walked a new direction the box stayed pointing the OLD way while the tool now hit the facing tile.
- **Cause:** the box targets `GetToolLocation(getMousePosition())` — the **`Vector2` overload** — and `getMousePosition()` returns the stale cursor position after the fade. The `bool` overload already reverts to the facing tile when `!wasMouseVisibleThisFrame`, but the box bypasses it.
- **Fix (`RightStickCursorPatches`):** prefix on `Character.GetToolLocation(Vector2, bool)` forces `ignoreClick = true` when `!Game1.wasMouseVisibleThisFrame` (local player, overworld) → the overload resolves the facing-direction tile (Character.cs:1202-1210). Only two direct callers of this overload (the box at `Farmer.cs:6367`, and tool use at `Game1.cs:12441` which already passes the facing-tile position when `!wasMouseVisibleThisFrame`), so it's safe.

### #77 Persist Zoom + Tool-Hit Options Across Cold Restart — DONE (v3.9.18, device-verified G Cloud 2026-06-05)
- **Symptom:** the in-game zoom + tool-hit-location options reset to defaults after a full game restart.
- **Cause (decompile-confirmed):** Android's `StartupPreferences` does NOT save `desiredBaseZoomLevel`, `alwaysShowToolHitLocation`, or `hideToolHitLocationWhenInMotion` — nothing persists them, so they reset each cold launch. The v3.8.26 "persists via StartupPreferences" assumption was wrong.
- **Fix:** AC stores the values in `ModConfig` (`SavedZoomPercent` / `SavedAlwaysShowToolHit` / `SavedHideToolHitWhenMoving`) and re-applies on `SaveLoaded` (`OptionsPagePatches.ApplySavedSettings`). Zoom is recorded when the user moves the slider (`ApplySliderValue`) and only re-applied if `> 0` (never clobbers the mobile default); tool-hit bools are snapshotted on Options/GameMenu close (`OnOptionsPageClosed`) and the config is written once there. `ApplyMobileZoom` made `internal` so the load hook can re-drive the PinchZoom pipeline. Files: `ModConfig`, `OptionsPagePatches`, `ModEntry`.

### #79 Contextual Cursor Sprite + session fixes — DONE (reduced scope) (dev v3.9.11→v3.9.16, device-verified G Cloud 2026-06-05)
- **#79 contextual cursor (v3.9.11):** the self-drawn right-stick cursor now shows the contextual sprite for interactable/inspectable **tiles** — `RightStickCursorPatches.ResolveContextualCursor()` reads the engine's per-tick hint flags (`isActionAtCurrentCursorTile`/`isSpeechAtCurrentCursorTile`/`isInspectionAtCurrentCursorTile`, set by `GameLocation.isActionableTile`) → **grab** (chest/mailbox/actionable objects), **look** (signs/inspectables, `Dialogue`/`Message` tiles), **talk** (`MessageSpeech` tiles), plus an un-petted farm animal → grab. The engine computes the full cursor in `Game1.drawMouseCursor` then resets `mouseCursor = cursor_default` (`Game1.cs:15688`) *before* our `RenderedHud` draw, so we replicate the pick from the (persisted) flags. Device-confirmed: chest → hand, Joja plastic cactus → magnifier. Spec/plan: `docs/superpowers/specs/2026-06-05-contextual-cursor-79-design.md` + `…/plans/2026-06-05-contextual-cursor-79.md`.
- **NPC speech-bubble/gift cursor: DROPPED (v3.9.13), deliberate.** The NPC path needs `Utility.checkForCharacterInteractionAtTile` *per render frame* (character iteration + held-item gift probe + a `checkForSpecialCharacterIconAtThisTile` side effect). It (a) never resolved villagers reliably — the engine only sets `cursor_talk` when the NPC has **pending dialogue** (`Utility.cs:4949-4967`) — and (b) was a real per-frame cost. Forage/furnace/shipping-bin **grab** is likewise out: they don't set `isActionAtCurrentCursorTile` (log-confirmed), so they'd need the heavy `canGrabSomethingFromHere` path. The tile-flag cursor is the shipped scope (user decision: parity isn't worth per-frame engine calls).
- **Logging floods removed (regression hygiene):** v3.9.12 debounced the cursor diagnostic (`[RStickCtx]` logs only on semantic-state change, killing the ~30 INFO lines/sec `[RStickDiag]`+`[RStickCtx]` flood); v3.9.14 throttled the **pre-existing** "Triggers raw" DEBUG flood in `ModEntry` (~18 lines/sec while a trigger is held → 1007 lines in one session; now logs only on rounded-value change). Both were Android main-thread synchronous-log-I/O stalls.
- **#54 trigger fast re-pull (v3.9.15):** lowered `TriggerReleaseConfirmTicks` 4→2 in `ModEntry`. At 4 the trigger had to dwell below the release threshold ~64ms before the next pull would register, dropping fast slot changes ("stuck when changing too quickly" — log-confirmed: 3 rapid LT pulls, 1 index change). At 2: log-verified **107/107 full pulls fired, 0 drops**. Tradeoff: a 2-3 tick analog dropout can double-move; bump toward 3-4 if spurious double-moves reappear.
- **#73 fertilizer placement (v3.9.16):** fertilizer (`Category == fertilizerCategory`, -19) plants into HoeDirt like seeds but was hitting the #68 craftable branch and drawing a translucent ghost; routed it through the seed **box-only** branch in `FurniturePlacementPatches` (+ `IsSpecialPlacementObject`) so it shows just the red/green target box. User-confirmed. Covers Basic/Quality Fertilizer, Speed-Gro, Retaining Soil, etc.
- **Debugging lesson (the big one):** the reported "input lag / stuck slot changes" were **NOT** caused by the #79 cursor — root causes were verbose-logging I/O floods + pre-existing #54 trigger tuning. The fix sequence above is the textbook trail: a logging hypothesis (v3.9.12) was *disproven* by the device test, which redirected to the trigger logic. **Diagnose with logging/measurement before stripping features.** (The NPC probe was removed mid-investigation as a wrong hang-suspect, but stays out on its own merits.)
...
- **Symptom:** `DialogueBox` question prompts (sleep Yes/No, NPC yes/no, festival/event choices) marked the selected response with a **yellow highlight box + faded (0.6-alpha) text** and opened with **nothing selected**. User wanted the console look: red outline + the menu finger cursor, drop the yellow.
- **Diagnosis:** AC didn't touch dialogue drawing — the yellow is **vanilla Android's own** rendering in `DialogueBox.draw` (`DialogueBox.cs:984-998`): selected → highlighted box `(267,256,10,10)` + 0.6-alpha text; unselected → plain box `(256,256,10,10)` + full-alpha text.
- **Fix (`Patches/DialogueBoxPatches.cs`, GMCM `EnableConsoleDialogueCursor` default on):** a `DialogueBox.draw` **postfix** overlays only the selected response — (1) covers the yellow box by re-drawing the plain box `(256,256,10,10)` over the identical rect (opaque 9-slice), (2) redraws the selected text at full alpha, (3) draws a **maroon `rgb(128,0,0)`** outline (4× `Game1.staminaRect` bars, 4px thick) around the box, (4) draws the menu/inventory finger cursor (`mouseCursors` **tile 44**, NOT tile 0) on the box's bottom line ~20% in from the right edge. Layout math (the `num` Y accumulation, box rect) mirrors `DialogueBox.draw` exactly so the cover lands pixel-for-pixel. Patching `draw` is safe on Android (only `releaseLeftClick` is the SIGSEGV landmine). The pre-existing `setUpQuestions` postfix pre-selects the top option (Yes).
- **ROOT CAUSE of the first "nothing changed" report (log-confirmed):** both the draw overlay AND the pre-selection gated on `gamepadControls && !lastCursorMotionWasMouse` (the game's own `setUpForGamePadMode` gate). Device log proved `gamepadControls=True` but **`lastCursorMotionWasMouse=True` on the controller** — Android delivers the controller confirm as a synthesized touch, so the flag reads as mouse/touch even on a gamepad, and the gate bailed every frame. Removing the flag gate (AC is controller-only scope → config toggle is the gate) fixed both. Same synthesized-touch family as #18. See memory `android-lastcursormotionwasmouse-true-on-controller`.
- **Tuning (device feedback, one commit each):** finger tile 0→44 (real menu cursor) + position to ~20%-from-right on the bottom line (v3.8.35); outline hue → maroon, picked via the new `tools/dialogue-outline-color-picker.html` live RGB preview rather than blind deploy cycles (v3.8.37 rust → v3.8.38 maroon). The earlier tool-hit pixel-sampler (`GetData` average) was dropped once the user gave a direct hue.
- **`Utility.To4` is Android-only** (absent on the PC reference DLL) — inlined the round-to-nearest-4 helper (`Utility.cs:5105`) locally, matching vanilla's selected-text alignment.
- **Device-verified:** opens on Yes, no yellow, maroon outline, menu finger bottom-right; D-pad up/down moves the indicator (log `sel` flips 0↔1); zero AC errors. Diagnostic removed v3.8.39. Spec: `docs/superpowers/specs/2026-06-04-dialogue-cursor-78-design.md`.
- **Files:** `Patches/DialogueBoxPatches.cs`, `ModConfig.cs`, `ModEntry.cs`, `tools/dialogue-outline-color-picker.html` (dev aid).

---

### #74 Console Drop-Blocker — FIXED v3.8.17 (device-verified G Cloud 2026-06-04)
- **Symptom:** dropping an item and instantly re-grabbing it (often before leaving the menu); on console you can't immediately re-collect your own drop.
- **Root cause (verified from the Android decompile):** the vanilla blocker is fully present but **dormant**. `Debris.DroppedByPlayerID` (`Debris.cs:89`) is read by the exclusion logic (`Debris.updateChunks` ~`:682` — while `timeBeforeReturnToDroppingPlayer > 0`, 1200ms `:111`, a farmer whose ID matches `DroppedByPlayerID` is rejected as a pickup/magnet target) but is **assigned nowhere in the entire decompile** (grep: one declaration, zero writes). `Game1.createItemDebris` doesn't set it either. So it stays `0`, the dropper is never excluded, re-pickup is instant. Pure Android omission.
- **Fix (Phase 1):** `TagAsPlayerDrop(Debris)` in `Patches/InventoryManagementPatches.cs` sets `DroppedByPlayerID.Value = Game1.player.UniqueMultiplayerID` on **deliberate player drops** — the controller drop (`DropHeldItem`) and the inventory-full failsafe (`CancelHold`) — capturing the `Debris` that `createItemDebris` returns. Re-engages the engine's own 1200ms exclusion; no custom timing. `DroppedByPlayerID` is a vanilla `NetLong` (present on the PC DLL), direct access, wrapped so a drop can never fail.
- **Deliberately NOT a blanket `Game1.createItemDebris` postfix** — that method also makes monster loot, crop harvest, and tree/rock-break debris; tagging those would wrongly block the player from collecting loot for 1.2s. Only player inventory-drops are tagged; loot/harvest stay instantly collectable (device regression-checked).
- **Toggle:** `EnableConsoleDropBlocker` (default true). **Device-verified:** user confirmed "working the same as the switch"; own drops no longer fly straight back, loot pickup unaffected.
- **Phase 2 parked (not built):** the user recalls that on **PC** you can drop an item and walk away indefinitely without it returning — a true "must leave the pickup radius" gate, stronger than the 1.2s timer (consistent with their Switch observation that 2 Iridium Bands' huge radius still never auto-re-grabs). The Android decompile only supports the 1200ms timer; Switch parity is met, so Phase 2 (a per-drop leave-radius gate) is parked unless PC-style behavior is specifically wanted. Design: `docs/superpowers/specs/2026-06-04-drop-blocker-74-design.md`.
- **Files:** `Patches/InventoryManagementPatches.cs`, `ModConfig.cs`, `ModEntry.cs`.

---

### #25b Slingshot Combat — FIXED v3.8.14→v3.8.16 (device-verified G Cloud 2026-06-04)
- **Symptom:** with a slingshot equipped, movement was blocked; the slingshot didn't aim/fire like console. Moving the stick alone fired it (no button), holding the tool button auto-fired repeatedly, release didn't cleanly fire, and the aim was mirrored 180° / defaulted straight down (the farmer would even turn to face the down-default on press).
- **NOT mod-caused — same subsystem as #25.** Android's tap-to-move/mobile-input layer (`Game1._mobileUpdateControlInput`, decompile lines 2446-2457) injects `useToolButtonPressed/Held/Released` from `currentLocation.tapToMove.mobileKeyStates`, which the touch weapon-control system derives from stick/tap motion. So the slingshot was driven by stick/touch instead of the clean console button path (the X-button branch at `Game1.cs:13640`/`13884`/`13689`). The slingshot's own engine (`beginUsing`/`tickUpdate`/`updateAimPos`/`onRelease`/`PerformFire`) was fine — it was being triggered wrong.
- **Root cause #1 — trigger hijack (fixed v3.8.15):** a postfix on `_mobileUpdateControlInput` (same method as #25 Stage 2) overrides the three use-tool refs, while a `Slingshot` is the current tool, to reflect ONLY the physical tool button (final `Buttons.X`, tracked in `GameplayButtonPatches` via new `GameUseToolPressedEdge`/`GameUseToolReleasedEdge`): `held → useToolHeld/useToolButtonPressed` (begin-once gated by `!UsingTool` at ~13884), `release edge → useToolButtonReleased` (fire once via the vanilla `onRelease` path at ~13689). This restores movement (no spurious `usingSlingshot`/`UsingTool` from stick touch), gives hold-to-draw, and fires exactly once on release. Device log confirmed: held→`usingSling=True` through the draw, release→one fire (`ammo -1`), `canMove=True` when not aiming.
- **Root cause #2 — legacy aim geometry (fixed v3.8.16):** Android defaults `Options.useLegacySlingshotFiring = true` (`Options.cs:2515`, the mobile default path; non-mobile defaults `false`). Legacy = old pull-back geometry: crosshair drawn OPPOSITE the stick (180° mirror), no facing-fallback so a centered stick defaults the aim straight down (farmer rotates to it), and `PerformFire` needs real aim distance (`backArmDistance > 4`) so a centered release does nothing. Switch runs non-legacy direct aim (`updateAimPos` facing-fallback → aim toward current facing; crosshair follows the stick; `backArm = 20*chargeTime` so a held release fires toward facing). Fix: force `useLegacySlingshotFiring = false` (guarded one-time write) in the same postfix while a slingshot is equipped, letting the engine's own non-legacy aim path run = console aim. Device log confirmed `legacy=False`; aim tracks the left stick (`aim` moves with `Lraw`); user confirmed "feels exactly the same as Switch now," including the draw animation.
- **Toggle:** `EnableSlingshotAim` (default true), scoped strictly to slingshot use (early-returns if `CurrentTool` isn't a `Slingshot`). No effect on any other tool/weapon. Mutually exclusive with #25's `ToolUsePatches` postfix (Hoe/WateringCan vs Slingshot) so the two `_mobileUpdateControlInput` postfixes never collide.
- **Diagnostics:** a `VerboseLogging`-gated once-per-tick `[Slingshot]` control-state line (`Patches/SlingshotAimPatches.cs`) — how both root causes were pinned and verified from the log (no playtest needed for the wiring). Kept, gated off for normal users.
- **Scope (v1):** console-parity left-stick aim only. The dual-stick stretch (right-stick aim-and-fire while walking, better-than-console) is deferred — see design spec `docs/superpowers/specs/2026-06-04-slingshot-combat-25b-design.md`; it needs a slingshot carve-out in `SuppressRightStickInOverworld` and ships as a separate toggle/commit.
- **LESSON:** any feature mixing a held tool/weapon button with movement on Android must reckon with the tap-to-move/mobile-input layer (`_mobileUpdateControlInput` → `TapToMove.mobileKeyStates`) — it injects use-tool button events from stick/touch and hijacks the clean controller path. #25 and #25b are the same villain. See memory `android-taptomove-clobbers-tool-on-move`.
- **Files:** `Patches/SlingshotAimPatches.cs` (new), `Patches/GameplayButtonPatches.cs` (button edges), `ModConfig.cs` + `ModEntry.cs` (toggle + GMCM + registration). Design: `docs/superpowers/specs/2026-06-04-slingshot-combat-25b-design.md`.
...
- **Symptom (public Nexus bug #1050718, "Can't resize the toolbar"):** dragging Options → "Toolbar Slot Size" did nothing to AC's toolbar.
- **Root cause (Android decompile + device diag v3.7.67):** the vanilla slider works fine — `OptionsPage` option id 148 (range 32–200) drives `Options.changeSliderOption(148)` → sets `toolbarSlotSize` / `Game1.maxItemSlotSize` / `Toolbar.Instance.itemSlotSize` + `resetToolbar()`. But `ToolbarPatches.Toolbar_Draw_Prefix` **fully replaces** `Toolbar.draw` with a **hardcoded `SlotSize = 64`** and never read `toolbarSlotSize`. So the slider moved but AC's draw ignored it. Device diag confirmed: uiViewport 1707×960, default `toolbarSlotSize` 80, slider feeds 32→200 straight through.
- **Fix (v3.7.68):** `ResolveSlotSize()` reads `Options.toolbarSlotSize` (Android-only → reflected) each draw, clamps to `[32, fit]` where `fit = (uiViewport.Width − margins − gaps)/12` so all 12 slots fit. Every `SlotSize` use (toolbar width/height, slot stride, selection box, slot bg) uses the resolved value. `DrawSlotItem()` scales each icon proportionally (`slotSize/64 × 1.0/0.8`) and mirrors vanilla `Toolbar.draw` by setting `Item.itemSlotSize` + `drawInToolbar` (both Android-only, reflected) and offsetting `drawInMenu`'s location, so the icon AND the stack-count/quality/gauge overlays (`DrawMenuIcons` reads `itemSlotSize`+`drawInToolbar`, both assume a 64px slot via `location+(32,32)`) stay centered as the slot grows. Per user direction the slider value is honored directly (default → device's 80px, not anchored to 64).
- **HUD clearance (v3.7.69):** at max the centered row overran the bottom-right energy bar (`viewport.Right − 64`, 48px wide; +56 for the mine health bar). Cap now reserves `HudSafeMarginX = 130` on BOTH sides (to stay centered): `maxFit = (uiW − 32 − 2·130 − gaps)/12` ≈ 114px on G Cloud. The centered bottom position is the binding case — the left-aligned top position extends less far right, so clearing the energy bar also clears the top clock (~284px wide).
- **Padding slider (v3.7.70/71):** vanilla "Toolbar Padding" (id 134, `Game1.toolbarPaddingX`, 0–160, default 0) is a horizontal shift for the vanilla toolbar — meaningless for AC's centered one, so it was dead. Repurposed as the gap from the docked edge (bottom, or top when the farmer is low on screen) via `ResolvePadding()`. v3.7.71 halved the travel (`PaddingGapDivisor = 2` → 0–80px) per user feedback that max was too far.
- **Diagnostic stripped v3.7.74.** All in `Patches/ToolbarPatches.cs`. Reflection used for every platform-differing member (`toolbarSlotSize`, `Item.itemSlotSize`, `Item.drawInToolbar`, `Game1.toolbarPaddingX`) — direct access would CS1061 against the PC DLL.

### #47 Missed Rewards Chest — NOT A BUG (resolved 2026-05-31, device-verified G Cloud)
- **Report:** after completing a CC room with unclaimed bundle rewards, the missed-rewards chest at tile (22,10) "never appears" on Android.
- **Finding (diagnostic v3.7.72/73 + device test):** the system works **correctly** on Android. With the condition set (area complete + a bundle reward ungrabbed), the device log showed the full chain succeed:
  - `checkForMissedRewards: missedRewardsChestVisible=True chestItems=36 | pendingRewards=[0] | cond{b0->area0(complete=True)}` — state machine flags it and fills the container.
  - `doShowMissedRewardsChest(isVisible=True) → (22,10) tile index=5 sheet=indoors2` — the tile **places successfully** (the `indoors2` tilesheet exists on the Android CC map; render works).
  - User walked up, the container was present, and pulled the reward out; afterward `pendingRewards=[] tile=NULL` — correctly cleared.
- **Why it read as "missing":** the container is the small **vanilla bag/giftbox sprite** (tile index 5), which is easy to overlook — the user confirmed they'd never noticed it in any playthrough. It looks identical to console/PC, so console parity is already met. Also, missed rewards only appear once the **whole area** is complete, not when a single bundle is pending — the original report's pending `[23,25]` (Vault bundles) were likely in an incomplete area, so the bag correctly stayed hidden.
- **Disposition:** no code change. Joins #48 (X/Y overlap) and #65 (bed bounce) as a "not a bug / works as designed" close-out. Diagnostic stripped v3.7.75. (Verified via the load/`resetSharedState` path; the natural room-completion path runs the same `doShowMissedRewardsChest` render and also calls `checkForMissedRewards` per the decompile, so it behaves identically.)
- **Test tooling:** `tools/seed-boiler-room.ps1` (inventory injector) was written during this investigation; the conclusive repro instead just flipped `areasComplete[0]` + `bundleRewards[0]` to true directly in the save XML (no room access needed).
...
### #18 Museum Donation + Rearrange Controller Support — v3.7.58→v3.7.66 (device-verified G Cloud)
- **Symptom:** in the museum donation menu (and the "Rearrange the museum?" menu) the controller couldn't drive donation/placement — the player flew blind, and rearrange walked the wrong slots.
- **Root cause #1 — NO VISIBLE CURSOR (the real blocker, NOT SnappyMenus).** The brainstorm spec premised that the console snap chain was gated on `Game1.options.SnappyMenus` being false on Android, so we'd need to force it true. **That premise was WRONG on G Cloud** and cost two wasted device builds (v3.7.58/59): diagnostics proved the `snappyMenus` FIELD and `SnappyMenus` PROPERTY were BOTH already `true` (field=True, gamepadControls=True, mouse buttons Released), and the mechanics ALL WORK once snappy is engaged — D-pad walks the snap, the game cursor (`Game1.getMouseX/Y`, which `selectItemAt`/`placeItem` actually read) tracks it, selection and placement fire. The ONLY thing missing was a **visible cursor**: `IClickableMenu.drawMouse` renders only when `mostRecentlyUsedControlType == ControlType.GAMEPAD`, but on Android the controller confirm is delivered as a synthesized touch, so the control type reads TOUCH and the built-in cursor is suppressed. Same class as the GeodeMenu / JunimoNote donation page.
- **Fix #1 (v3.7.61, the actual fix):** a `MuseumMenu.draw` postfix draws the snap cursor ourselves (tile 44 at `Game1.getMouseX/Y`), bypassing `drawMouse`'s control-type gate. Gated on the fade/exit states so we don't paint a cursor on black.
- **Root cause #2 — REARRANGE walked the hidden inventory.** Rearrange opens the same `MuseumMenu` (via `OpenRearrangeMenu`) but its `receiveKeyPress` only grid-navigates the placed pieces when `heldItem != null || reOrganizing`, and vanilla never sets `reOrganizing` — so the D-pad snapped through the hidden donatable-inventory slots instead of the museum grid.
- **Fix #2 (v3.7.63):** a `MuseumMenu` ctor postfix sets `reOrganizing = true` for rearrange-mode menus. `reOrganizing` is read ONLY by that nav gate, so it's surgical; the first directional press jumps the cursor onto a free grid tile (vanilla `getFreeDonationSpot` path), then presses walk the pieces and A picks up / swaps.
- **SnappyMenus force kept as cross-device insurance.** The `snappyMenus` field + an `Options.get_SnappyMenus` getter postfix force snappy true while our menu is open (set in the `OpenDonationMenu`/`OpenRearrangeMenu` prefix before construction, restored via `ModEntry.OnMenuChanged`). Confirmed a **no-op on G Cloud** but harmless, and genuinely needed on any device where the field really is false.
- **PC-DLL reflection landmine:** `MuseumMenu.rearrangeMode` and `.reOrganizing` are **Android-only fields — absent from the PC DLL** the project compiles against; direct access is CS1061 and gets the whole mod rejected as incompatible. Reflect them via `AccessTools.Field`. Likewise `Options.SnappyMenus` is getter-only on PC → write the lowercase `snappyMenus` **field** (read/write on both platforms) instead of the property.
- **Right-stick pan tried and DROPPED (v3.7.64 → reverted v3.7.65).** Added a `MuseumMenu.update` postfix that panned the viewport from the right stick via `Game1.panScreen` for the cramped far-right/top tiles at default zoom. On device it broke donation — a thick black bar on the bottom/right during fade-in, offset inventory, dead cursor/A — and made rearrange unreachable (driving `panScreen` desynced the viewport the menu + snap system depend on). Reverted entirely; players who dislike the default view can zoom in/out before donating.
- **Diagnostic scaffolding stripped (v3.7.66):** the v3.7.60 log-only postfixes (ctor/receiveKeyPress/receiveLeftClick/update + the `DiagState` helper) and the `[MuseumMenu/diag]` logs were removed once both flows were device-confirmed. They were all log-only on SAFE methods — deliberately NOT `releaseLeftClick`, the Android SIGSEGV landmine from #19.
- **LESSON:** when a controller menu "doesn't work" on Android, confirm whether the *mechanics* work before assuming a gating/nav bug — instrument first. Here everything worked except the cursor sprite; the fix was a few lines of `draw`, not a nav rewrite. Don't relitigate the SnappyMenus theory (see memory `museum-donation-controller-rootcause`).
- **Toggle:** `EnableMuseumDonationController` (default true). **File:** `Patches/MuseumMenuPatches.cs` (single file); restore wired in `ModEntry.OnMenuChanged`.
...
- **Two real defects + one misread of console UX:**
  - **(A) Constructor asymmetry.** `LetterViewerMenu(string)` (decompile line 89-98, used by `Game1.drawLetterMessage` → kill list) is missing the snap block its sibling `LetterViewerMenu(string, string, bool)` runs at lines 176-185. Without it, `currentlySnappedComponent = null` on entry; the cursor isn't placed on the forward arrow; A does nothing until `IClickableMenu`'s lazy snappy-nav fires on first joystick input.
  - **(B) Vanilla `snapToDefaultClickableComponent` is visibility-blind.** Decompile line 524-538 unconditionally picks `forwardButton` (id 102) for non-interactable letters regardless of `visible`. On the last page `forwardButton.visible = false`. So every page-flip onto the final page leaves `currentlySnappedComponent` pointing at an invisible button — A targets nothing visible, cursor renders at an empty position. Symmetric problem on page 0 (`backButton.visible = false`).
  - **(C) Misread of "no cursor."** Original spec assumed the cursor sprite would render via vanilla `Game1.drawMouse` once snap was correct. v3.7.18 device test showed it does NOT — `LetterViewerMenu.draw` (decompile lines 744-793) never calls `drawMouse`, and Android's outer-loop drawMouse is suppressed in controller mode. v3.7.18 added a draw-postfix that rendered the cursor sprite ourselves (same pattern as #17 / #40a / JunimoNoteMenu donation page). On test, the user redirected: console SDV doesn't show a cursor at all — snap *is* the indicator. The cursor sprite was useful only as a debug aid to verify the snap. See memory note `feedback_console_ux_no_cursor`.
- **Fix arc:**
  - **v3.7.15** — diagnostic-only patch on both ctors, `receiveGamePadButton`, and `update`. Confirmed `snapped=null` after the `(string)` ctor and `snapped=id=102` after vanilla snap nav kicked in on first input. Smoking gun for (A).
  - **v3.7.16** — `CtorString_Postfix` mirrors the sibling overload's snap block. `populateClickableComponentList()` + `snapToDefaultClickableComponent()` + the single-page `myID = -100` zeroing.
  - **v3.7.17** — same fix as v3.7.16 with diagnostic logging retained, to capture device-side state for future regressions.
  - **v3.7.18** — added `Update_Postfix` (re-snap when the snapped arrow becomes invisible after a page flip — picks accept-quest > item-grab > opposite arrow as the fallback) and a `Draw_Postfix` that drew the cursor sprite. User-tested: snap and re-snap correct, cursor visible, but on A-press `Game1.lastCursorMotionWasMouse` flips True for one frame and the cursor blinked off until the next joystick input — a known gate quirk we deliberately did not paper over.
  - **v3.7.19 (final)** — removed `Draw_Postfix`. Kept `CtorString_Postfix` (snap fix) and `Update_Postfix` (re-snap fix) and retained the diagnostic logging in both. Console-parity: no cursor sprite, snap is the selection indicator.
- **Scope:** One file covers both reported menus (kill list and multipage mailbox letters) because both flow through the same `update` / `draw` / `snap` paths. The `(string, string, bool)` overload already has its own vanilla snap block, so it inherits only the `Update_Postfix` re-snap fix.
- **File:** `Patches/LetterViewerMenuPatches.cs`, registered in `ModEntry.cs`.
- **Verified (Ayaneo Pocket Air Mini, v3.7.19):** kill list opens with snap on forward arrow, A turns the page, snap moves to back arrow on page 2, A returns to page 1, B closes. Multipage mailbox letters confirmed working. Title screen and save-select screen no longer show the cursor sprite (TitleMenuPatches deleted; LoadGameMenuPatches never had a draw patch).
- **Key lessons:**
  - Vanilla `snapToDefaultClickableComponent` for `LetterViewerMenu` doesn't check `visible` — any caller that relies on it for re-snap after page flips needs a visibility-aware shim.
  - **Console UX = no visible cursor sprite.** Don't ship `Draw_Postfix` cursor patches as a default; they were a misread. Snap is the selection indicator. See memory note `feedback_console_ux_no_cursor`. This invalidates the "draw the cursor ourselves" framing in older specs (`2026-05-14-title-menu-cursor-design.md`, `2026-05-15-load-game-cursor-design.md`).
  - `TitleMenuPatches.cs` deleted: vanilla TitleMenu snap state is correct on its own; removing the draw patch leaves the menu cursorless (correct console UX).
